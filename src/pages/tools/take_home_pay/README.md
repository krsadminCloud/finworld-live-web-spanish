...
- Left column (inputs): stacked fields for fast entry
  ...
  - Breakdown list with zebra rows, percent-of-gross chips for each deduction/tax; every tax row (‘Federal’, ‘State’, ‘FICA’, ‘Local’) includes a “Details” link that explains how the value was computed (standard deduction, state deduction, FICA components, and adjusted income×local rate).
